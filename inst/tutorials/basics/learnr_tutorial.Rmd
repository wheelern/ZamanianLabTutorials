...
For the purposes of this tutorial, we will be loading packages as needed, and I will point out what the package is often used for and what the new function that we're using does. When you begin writing your own R scripts, it's good practice to load all of the packages you'll need at the top of the script, before you start any analysis. If you get halfway through the analysis and realize you need another package, add it to the top.

Ok, now that that's over with, we'll actually import the data. Your data will **almost** always be in the form of a CSV file - a text file with comma-separated values. To import the data, we'll use a function from the `readr` package called `read_csv()` which will read all the data from your CSV file and store it in an objecte called a `data.frame`. Actually, it will load it into a special type of object called a `tibble`, but that's more detail than we need right now. A `data.frame` is basically a set of columns (aka variables) sorted side-by-side. Each column **must** have the same number of rows (aka observations) and **must** contain the same data type (for example, integers, numbers, characters, etc.). Load some of my example data with the code below. 

```{r import, exercise=TRUE, message=FALSE}
raw_data <- read_csv("~/Box/ZamanianLab/LabMembers/Nic/R_workshop/1_rawdata/ce_chemotaxis.csv") %>%
  glimpse()
raw_data
```

In the code block we just ran, notice that the `tibble` is assigned a name, `raw_data` using the assignment operator `<-`. Notice also the `%>%` operator. This is called a pipe, and it's something that is specific to the `tidyverse`, of which `readr`, `dplyr`, `tidyr`, and `ggplot2` are a part. The pipe allows us to sequentially run manipulations on the `tibble` without repeatedly assigning it a new name. In this case, it allows us use to run the `glimpse` function, which shows us a little bit about our `tibble`. Without the pipe, you would have to run the command like this: `glimpse(raw_data)`. Finally, we can view the data in a table format by simply running `raw_data`.

The `glimpse()` command showed us an easily readable summary of the `tibble` that we just created. It tells us that we have 156 observations (rows) and 9 variables (columns). These columns are listed with their name (Date, Strain, etc.), type (dbl, chr, etc.), and a few observations. You won't often use `glimpse()` when doing interactive analyses, but it's useful for the purposes of this tutorial.

Use the code block below to import your own data by replacing the `...` with the path and file name of your choice. (Note: you will have to copy your test data to my lab folder at `~/Box/ZamanianLab/LabMembers/Nic/R_workshop/1_rawdata`.) If you've done it correctly, running `test_data`  will show you a bit of your imported data. If not, the command will print an error, something like `'your data' does not exist in current working directory`.

```{r test_import, exercise=TRUE}
test_data <- read_csv("~/Box/ZamanianLab/LabMembers/Nic/R_workshop/1_rawdata/...")
test_data
```

We now have our data imported and stored in the prefered `tidyverse` object, the `tibble`, and we're ready to tidy, analyze, and visualize our data.

### Data Tidying

After importing the data, we have a number of questions to answer. Do we need to calculate any new variables? Are there extraneous observations or variables that ought to be removed? Is the data in a format that is conducive to plotting? The answers to these questions will dictate the steps you take after importing.

Take another look at the data below, which was generated by running *C. elegans* chemotaxis experiments. We have some metadata that describes the experiment: Date, Strain, Cue, Control, Notes. Then we have important measured data Cue_N, Control_N, Outside, and Total. These 4 variables contain integers because they are counts - I counted the number of worms at the Cue, at the Control, at neither the cue or control (Outside). I then summed these to get a total count. The data from the experiment is complete, but none of this data is actually what I'm interested in; I'm interested in a summary statistic called the Chemotaxis Index. The CI is a single number between -1 and 1 that tells me how likely I was to find a worm at the test cue. The CI is calculated by the following formula: $CI = \displaystyle \frac{Cue_N - Control_N}{Total_N}$. 

```{r message=FALSE}
read_csv("~/Box/ZamanianLab/LabMembers/Nic/R_workshop/1_rawdata/ce_chemotaxis.csv")
```

To create a new variable `CI`, we run the following code, which will add a new variable, calculate the CI, and add each CI as a new observation:

```{r calculate-setup, echo=FALSE, message=FALSE}
raw_data <- read_csv("~/Box/ZamanianLab/LabMembers/Nic/R_workshop/1_rawdata/ce_chemotaxis.csv")
```

```{r calculate, exercise=TRUE, message=FALSE}
raw_data <- mutate(raw_data, CI = (Cue_N - Control_N) / Total)
raw_data
```

You'll notice a few new things in this code block. First, you'll notice the new function `mutate()`, which is a part of one of the most essential packages of the `tidyverse`, `dplyr`. `dplyr` contains a host of functions that will allow you to "ply" your data (get it? It's a data plier). You can find a handy `dplyr` cheatsheet [here](https://www.rstudio.com/wp-content/uploads/2015/02/data-wrangling-cheatsheet.pdf). `mutate()` creates a new column/variable, and it uses values from other variables (and calculations on those values) to construct the new column. In this case, we're telling `mutate()` to create a new column named `CI` and to fill this column with values by taking the difference of `Cue_N` and `Control_N` and dividing it by `Total`. `mutate()` will perform this calculation in a vectorized manner, which for simplicity's sake means it will perform the operation at once on the entire vector (i.e. the entire column) instead of row-by-row. Recall that one of the requirements of the `data.frame` or `tibble` is that each variable must have the same number of observations. This is because of the way that vectorized functions work. If `Cue_N` had more observations than `Control_N`, what would `mutate()` do for the observations with missing data? The function would fail, so keeping your data in a `tibble` will ensure all the downstream vectorized functions will work as expected.

Check out the new `tibble` that was generated. Do you see how the `mutate()` function created a new variable? `mutate()` will most likely be one of your most often used function, so make sure you know how it works!

Now that we have all the variables we will need for analysis and plotting, we need to restructure the data. There are two ways to structure data - long and wide. Wide data is how most people intuitively store data, with observations and variables spread across multiple columns. Wide data is easier to enter manually and often easier for a human to visualize. However, data analysis in R is better done with long data - where each column is a variable with the same data type and each row is a single observation.

You'll see that `raw_data` is organized such that each observation (row) contains all the data measured from a single chemotaxis plate, and each variable (column) contains either some metadata about the experiment or a recorded value from that plate. Because each row contains 4 observations, this data is in a wide format and we will need to convert it to long format for all the downstream analyses and visualizations. To do that, we use a new `tidyr` function called `pivot_longer()`. See below:

```{r long-setup, echo=FALSE, message=FALSE}
raw_data <- read_csv("~/Box/ZamanianLab/LabMembers/Nic/R_workshop/1_rawdata/ce_chemotaxis.csv") %>%
  mutate(CI = (Cue_N - Control_N) / Total)
```

```{r long, exercise=TRUE, message=FALSE}
long_data <- raw_data %>%
...
```{r save-setup, echo=FALSE, message=FALSE}
raw_data <- read_csv("~/Box/ZamanianLab/LabMembers/Nic/R_workshop/1_rawdata/ce_chemotaxis.csv")
...
```{r save, exercise=TRUE, message=FALSE}
tidy_data <- long_data
saveRDS(tidy_data, "~/Box/ZamanianLab/LabMembers/Nic/R_workshop/2_tidydata/tidy_data.rds")
```

Notice that I first renamed the long data to `tidy_data`. In reality this was unnecessary; I could have saved `long_data` directly. However, with real life data, there might be more tidying that needs to happen after getting the data in long format, and only the completed tidy dataset should be saved as an RDS file.
...
test_tidy_data <- test_long_data

saveRDS(tidy_data, "~/Box/ZamanianLab/LabMembers/Nic/R_workshop/2_tidydata/...")
```
...
tidy_data <- readRDS("~/Box/ZamanianLab/LabMembers/Nic/R_workshop/2_tidydata/tidy_data.RDS")
```
...
```{r filter_test-setup, echo=FALSE}
tidy_data <- readRDS("~/Box/ZamanianLab/LabMembers/Nic/R_workshop/2_tidydata/tidy_data.RDS")
```

```{r filter_test, exercise=TRUE}
...
As stated above, we first used the `group_by()` function to group our filtered data by Strain, Cue, and Measurement. Next, we used the `summarize()` function to create three new variables from each of the defined groups. R has  built-in functions for calculating the mean and standard deviation, but we had to manually calculate the standard error of the mean. You'll notice that the resulting `tibble` has fewer observations - one for every group. Check out the `dplyr` cheatsheet for other useful summary functions.
...
In this summary, we did not group by the experiment date, so variation that we see through the standard deviation and standard error of the mean is a measure of variation across all the experiments that I ran. We might also be interested in the technical variation between individual chemotaxis plates (this data was collated from 3 independent biological replicates for each cue, and each biological replicate included 5 technical replicates per strain.) To get a measure of technical variation, you will have to change the grouping strategy. Use the code block below to get this summary data.
...
We're nearly there! Instead of randomly jittered points we used a new function from the package `ggbeeswarm` to arrange the points in more predictable fashion. We also added colors to the make differentiation of the strains absolutely clear. Second, we added a new layer, `geom_pointrange()` which plots the mean and standard errors that we calculated in `ci_summ` (Note: `geom_pointrange()` requires three aesthetics - y, ymin, and ymax. To calculate these we passed the `ci_summ` data instead of `data_filt` which was passed in `ggplot()`). Next, we modified the y-axis with the `scale_y_continuous()` function, which accepted the `limits` and `breaks` arguments. The `limits` are self-explanatory, but the solution for the `breaks` argument - where the labels and ticks will occur, is a bit more complex. Here, we used the `seq()` function to create a vector of numbers from -0.5 to 1, with breaks every 0.25. If you run `seq(from = -0.5, to = 1, by = 0.25)` alone you can see the output, and if you run `?seq()` you can see the documentation for this function. In fact, you can read the documentation of every single function that is loaded by running the function with a `?` prepeneded, which is how I knew about the three different aesthetics required by `geom_pointrange`. (I am constantly looking up the documentation of functions, even functions that I use every day. Don't feel bad if you have to look something up! Documentation (and Google) is your friend.) We next used `labs()` to redefine the y-axis label, and `theme()` to remove the legend. `theme()` is an incredibly powerful function that allows you to customize almost every imaginable feature of the plot. We'll get to a longer discusion on `theme()` shortly.
...
data_filt <- mutate(data_filt, Strain = factor(Strain, levels = c("N2", "CX10", "PR678", "ZAM13", "ZAM14")))
...
```{r theme, exercise=TRUE, message=FALSE}
library(cowplot)
...
```{r theme2-setup, echo=FALSE, message=FALSE}
tidy_data <- readRDS("~/Box/ZamanianLab/LabMembers/Nic/R_workshop/2_tidydata/tidy_data.RDS")
...
```{r theme2, exercise=TRUE, message=FALSE}
ci_plot <- ci_plot +
...
```{r stats-setup, echo=FALSE, message=FALSE}
tidy_data <- readRDS("~/Box/ZamanianLab/LabMembers/Nic/R_workshop/2_tidydata/tidy_data.RDS")
...
```{r stats, exercise=TRUE, message=FALSE}
library(ggpubr)
...
`ggpubr` is a great package that enables easy addition of statistical tests to your plot. To do so, you first have to make a list of vectors that defines the pairwise comparisons to perform, and pass this list to the `comparisons` argument of `stat_compare_means()`. The other arguments define the statistical test used, what labels are added, and where those labels are plotted. In order to make room for these labels, we had to redefine the y scale to extend the limits.
